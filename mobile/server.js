...
function mapDeezerTrack(track) {
    ...
    return {
        ...
        // Artist name goes where "genres" normally shows — Deezer doesn't
        // give a genre per track without extra calls, and the artist is
        // what actually belongs under a song title anyway.
        genres: track.artist?.name ? [track.artist.name] : [],
        description: track.album?.title ? `From the album "${track.album.title}"` : '',
        // Deezer's public API doesn't expose per-track ratings — this is
        // genuinely absent from the data, not a bug. "Highest/Lowest
        // Rated" sort won't meaningfully reorder music as a result.
        averageRating: null,
        ratingCount: 0,
        // Chart/search results don't include a release date — only the
        // full track detail endpoint does, so this stays blank until
        // hydrated (detail page open, or playlist reload).
...
// Same idea as buildPopularPool, but for RAWG games. Unlike TMDB, we
// don't pre-fetch each game's description here — that would mean ~600
// extra RAWG calls up front (very likely to hit rate limits, which is
// exactly what caused the earlier "<!DOCTYPE" error). Instead,
// descriptions are hydrated lazily, only for the ~20 games in whatever
// page is actually being viewed, and cached on the pool item afterward
// so the same game is never re-fetched twice.
...
app.get('/api/getgames', async (req, res) => {
    try {
        ...
        // RAWG's list endpoint doesn't include a description, so we fetch
        // each game's detail for that one field. Fine for ~20 items;
        // worth caching if this page grows.
